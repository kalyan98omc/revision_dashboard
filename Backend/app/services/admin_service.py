...
Use clinical correlations when relevant. Format responses clearly with headers and bullet points.""",
    ...
    def retry_document(doc_id: str) -> dict:
        """Retry uploading a document to OpenAI (for failed/stuck documents)."""
        doc = db.session.get(Document, doc_id)
        if not doc:
            raise NotFoundError("Document not found")

        if not os.path.exists(doc.file_path):
            raise ServiceError("Document file not found on disk")
    ...
    def check_document_status(doc_id: str) -> dict:
        """Check detailed status of a document in OpenAI."""
        doc = db.session.get(Document, doc_id)
        if not doc:
            raise NotFoundError("Document not found")

        status_info = doc.to_dict()
